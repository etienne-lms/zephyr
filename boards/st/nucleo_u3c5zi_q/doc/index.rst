...
with TrustZobne security enable ('ns' variant) where TF-M is the embedded
...
Building Zephyr embedded images for the 'ns' vairant uses standard
...
on the board. Disabling TZEN must be done by regrssion from RDP level 1 to
RDP level 0. The procedure is described in Application Note `AN5347`_,
section 9 "TrustZone deactivation". It is recommended to carefully read this
document.
